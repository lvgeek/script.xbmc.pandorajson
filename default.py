...
dlg = xbmcgui.Dialog()
dlg.notification("PANDORA", "Loading...", xbmcgui.NOTIFICATION_INFO, 250)
...
#        else: 
    ...
    def auth(self):
        dlg = xbmcgui.Dialog()
        dlg.notification("PANDORA", "Logging In...", xbmcgui.NOTIFICATION_INFO, 250)
    ...
    def Quit(self):
        __settings__.setSetting("LastStation", self.curStation)
        if self.player and self.player.timer\
                and self.player.timer.isAlive():
            self.player.timer.stop()
        #if self.gui != None:
